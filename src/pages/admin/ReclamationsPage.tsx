...
export default function ReclamationsPage() {
  ...
  const handleDeleteMany = async () => {
    ...
    try {
      ...
      alertSuccess(`${itemsToDelete.length} réclamations supprimées`);
      if (typeof refresh === "function") {
        await refresh();
      }
    } catch (error) {
  ...
  return (
    <PageLitLayout title="Gestion des réclamations">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Gérez toutes les réclamations clients</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-4 mt-6">
            ...
            <div className="bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-4">
              <p className="text-xs text-red-700/70">Rejetée</p>
              <p className="text-2xl font-bold text-red-700">{stats.rejetee}</p>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <ReclamationsList
              ...
              onSelectReclamations={(selected) => setItemsToDelete(selected.map((r) => r.id || r._id || ""))}
              selectable={false}
              selectActions={[
                {
                  label: "Supprimer",
      ...
      <DeleteConfirmationModal
        ...
        message={`Supprimer ${itemsToDelete?.length} réclamations ?`}
      />
    </PageLitLayout>
  );
}